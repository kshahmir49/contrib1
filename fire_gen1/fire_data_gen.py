...
sensor_locs = np.array([[int(x),int(y)] for x in [aoi_size/4, aoi_size/2, aoi_size*3/4] for y in [aoi_size/4, aoi_size/2, aoi_size*3/4]])
print(sensor_locs)
    ...
    for i in range(data_size):
        if t==0:
            ...
            if np.count_nonzero(image == 1) < (aoi_size**2) * 0.8:
                ...
                print("Time out, starting new fire")
                t = 0
...
# plt.show()
np.save("fire_gen/input_wofire.npy", input)
np.save("fire_gen/output_wofire.npy", output)
np.save("fire_gen/sensor_values_wofire.npy", sensor_values)
